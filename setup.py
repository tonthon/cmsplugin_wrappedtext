...
def generate_setuptools_kwargs_from_setup_cfg():
    ...
    for arg in SETUP_ARGS:
        ...
        if arg in DICT:
            if arg == 'package_data':
                if config.has_section('package_data'):
                    datas = dict([(pkg, [datas])for pkg, datas in config.items('package_data')])
                    kwargs[arg] = datas

            if arg == 'package_dir':
                if config.has_section('package_dir'):
                    datas = datas = dict(config.items('package_dir'))
                    kwargs[arg] = datas
        else:
            try:
